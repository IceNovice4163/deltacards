...
from targeting import NoTargetsError, Predicate, Var
...
class Arg(Generic[T]):
    def __init__(self, *, default: Any = _MISSING, many: bool = False, raw: bool = False):
...
class Action:
    ...
    def __rshift__(self, other: Any) -> 'EffectBase':
        from effects import Seq, effectify
    ...
    def to(self, to: Any, else_: Any | None = None) -> 'EffectBase':
        from effects import Then, effectify
    ...
    def store_result(self, var: Var) -> 'EffectBase':
        ...
        from effects import StoreResult
    ...
    def resolve(self, ctx: ActionContext, **kwargs) -> dict[str, Any]:
        ...
        for name, arg_def in self._arg_defs.items():
            ...
            origin = typing.get_origin(arg_def.expected_type)
            expects_list = (origin is list)

            if (not arg_def.many) and isinstance(value, list) and not expects_list:
                if len(value) == 0:
